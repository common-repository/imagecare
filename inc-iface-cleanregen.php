...
<form method="post" action="">
	...
	<p><input type="submit" class="button hide-if-no-js" name="letsgo" value="<?php _e( 'Clean & Regenerate', 'cleanregen' ) ?>" /></p>
	<noscript><p><em><?php _e( 'You must enable Javascript in order to proceed!', 'imagecare' ) ?></em></p></noscript>
</form>
<?php
	} else {
		if($this->check_before_do('cleanregen')) {
...
?>
	...
	<p>
		...
		<?php printf( __( 'Resize failures: %s', 'imagecare' ), '<span id="imagecare-debug-failurecount">0</span>' ); ?><br />
		<?php printf( __( 'Previous size: %s',   'imagecare' ), '<span id="imagecare-debug-previoussize">0</span>' ); ?><br />
		<?php printf( __( 'Current size: %s',    'imagecare' ), '<span id="imagecare-debug-currentsize">0</span>'  ); ?><br />
		<?php printf( __( 'Saved: %s',           'imagecare' ), '<span id="imagecare-debug-saved">0</span>'        ); ?>
	</p>
		...
		jQuery(document).ready(function($){
			...
			function do_it(id) {
				jQuery.ajax({
					type: 'POST',
					url: ajaxurl,
					data: { 'action': 'imagecare_cleanregen', 'id': id },
